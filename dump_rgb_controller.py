...
from utils.keyboard_map import ALIASES as _ALIASES  # 吏꾨떒/蹂닿컯??
...
def _refresh_device_leds(dev=None):
    """?μ튂??理쒖떊 LED ?곹깭瑜?媛깆떊?쒕떎.
    - ?몄옄濡??μ튂 媛앹껜媛 二쇱뼱吏硫??대떦 媛앹껜瑜??덈줈怨좎묠
    - ?놁쑝硫?湲濡쒕쾶 kb瑜??덈줈怨좎묠
    """
    target = dev or kb
    try:
...
def safe_set_direct_and_sync(kb):
    """?쒖뼱 ??1?? 紐⑤뱶 ?꾪솚 ??硫뷀??곗씠??媛깆떊 ???꾩껜 ?щ툝??珥덇린??""
    try:
        kb.set_mode("direct")
    except Exception:
        pass
...
def _ensure_stage_labels(km: RGBLabelController) -> None:
    """諛⑺뼢???쇰꺼(up/down/left/right)???꾨씫??寃쎌슦 JSON??吏곸젒 ?쎌뼱 蹂닿컯.
    - ?쇰? ?섍꼍?먯꽌 蹂꾩묶 留ㅼ묶 ?꾨씫 ???쒖떆媛 ?꾪? ??蹂댁씪 ???덉쑝誘濡??덉쟾留??쒓났.
    """
    try:
...
def get_key_color(label: str, fresh: bool = True) -> List[Tuple[int, int, int]]:
    ...
    if idx is None:
        raise KeyError(f"'{label}' ?쇰꺼???대떦?섎뒗 LED瑜?李얠쓣 ???놁뒿?덈떎.")
