...
"""
# =========================
# CPU/Parser Î™ÖÎ†π???ïÎ¶¨ (signed Î≤ÑÏ†Ñ)
# =========================
#
# ??Í∏∞Î≥∏ Í∑úÏπô
# - Î™®Îì† Í≥ÑÏÇ∞?Ä signed 8ÎπÑÌä∏(-128..127)Î°??ôÏûë.
# - ?åÎûòÍ∑?
#     Z (Zero)      : Í≤∞Í≥ºÍ∞Ä 0?¥Î©¥ 1
#     N (Negative)  : Í≤∞Í≥ºÍ∞Ä ?åÏàò(Î∂Ä?∏ÎπÑ??1)Î©?1
#     V (oVerflow)  : signed overflow Î∞úÏÉù ??1 (ADD/SHL/SUB/CMP ??
# - PC??"?§Ïùå Î™ÖÎ†π???ÑÏπò". Î≥¥ÌÜµ ??Ïπ∏Ïî© Ï¶ùÍ??òÏ?Îß?
#   ?êÌîÑ/Î∂ÑÍ∏∞(JMP, BEQ ??Í∞Ä ?§Ìñâ?òÎ©¥ ÏßÅÏ†ë Î∞îÎÄ?
# - Ï¶âÏãúÍ∞??úÍ∏∞: #10, #0x1F, #-3 ??Î™®Îëê ?àÏö©.
#
# ??Í∏∞Î≥∏ Î™ÖÎ†π??
# - NOP / LABEL / HALT / PRINT (?ôÏùº)
#
# ??Í∞??£Í∏∞ / ?îÌïòÍ∏?/ ÎπºÍ∏∞
# - MOVI dst, #imm         ; dst ??imm (signed Î≤îÏúÑÎ°??òÌïë ?Ä??
# - MOV  dst, src          ; dst ??src
# - ADDI dst, #imm         ; dst ??dst + imm (Z/N/V Í∞±Ïã†)
# - ADD  dst, src          ; dst ??dst + src (Z/N/V Í∞±Ïã†)
# - SUBI dst, #imm         ; dst ??dst - imm (Z/N/V Í∞±Ïã†)
# - SUB  dst, src          ; dst ??dst - src (Z/N/V Í∞±Ïã†)
#
# ??ÎπÑÌä∏ ?∞ÏÇ∞ (Z/N Í∞±Ïã†, V=0)
# - AND/OR/XOR
#
# ???úÌîÑ??
# - SHL dst                ; ?∞Ïà†???òÎ????ºÏ™Ω ?úÌîÑ??ÏµúÏÉÅ??ÎπÑÌä∏ Î≥Ä?îÎ°ú V ?êÎã®)
# - SHR dst                ; ?∞Ïà† ?úÌîÑ??ASR, Î∂Ä???†Ï?). V=0
#
# ??ÎπÑÍµê / Î∂ÑÍ∏∞ (signed)
# - CMP a, b               ; a-b??signed Í≤∞Í≥º Í∏∞Î∞ò?ºÎ°ú Z/N/V Í∞±Ïã†(Í≤∞Í≥º??Î≤ÑÎ¶º)
# - CMPI a, #imm
#
# - JMP label              ; Î¨¥Ï°∞Í±?Î∂ÑÍ∏∞
# - BEQ/BNE                ; Z=1 / Z=0
# - BMI/BPL                ; N=1 / N=0
# - BVS/BVC                ; V=1 / V=0
#   (?òÏúÑ?∏Ìôò) BCS/BCC     ; V=1 / V=0 Î°??¥ÏÑù
"""
...
class CPU:
    def __init__(self, *, debug: bool = False, mem=None, interactive: bool = False, use_isa: bool = True) -> None:
        ...
        # ?åÎûòÍ∑? Zero / Negative / oVerflow / Carry
        self.flags: Dict[str, int] = {"Z": 0, "N": 0, "V": 0, "C": 0}
        self._pc_overridden: bool = False

        self.interactive = interactive                         # ???§ÌÖù ?§Ìñâ ?åÎûòÍ∑?        self._continue_run = False                             # ??'c' ?ÖÎ†• ??Í≥ÑÏÜç ÏßÑÌñâ
        self.use_isa = use_isa                                 # ??ISA Î™®Îìú ?¨Ïö© ?¨Î?

        # ISA instruction stream (when use_isa=True)
        self._isa: list[AsmInsn] = []
        # Control-plane integration (LED-gated run loop)
        # Í∏∞Î≥∏Í∞?False: ?∏ÌÑ∞?ôÌã∞Î∏?Î™®Îìú?êÏÑú ÏΩòÏÜî ?ÖÎ†• ?∞ÏÑ†
    ...
    def load_program(self, lines: List[str], *, debug: bool | None = None) -> None:
        ...
        # Ï¥àÍ∏∞ ?åÎûòÍ∑??ÅÌÉúÎ•?LED??1??Î∞òÏòÅ(Off ?ÅÌÉú?ºÎèÑ Î∞îÎ°ú Î≥¥Ïù¥Í≤?
        try:
    ...
    def _maybe_pause(self) -> None:
        """interactive Î™®ÎìúÎ©? ???∞ÏÇ∞ ?ùÎÇ† ???¨Ïö©???ÖÎ†• ?ÄÍ∏?
        [Enter]=???§ÌÖù, 'c'=?∞ÏÜç ?§Ìñâ, 'q'=Ï¶âÏãú Ï¢ÖÎ£å"""
        ...
        if self._continue_run:
            # ÎπÑÏ∞®?? Î∞±Í∑∏?ºÏö¥??Î™ÖÎ†π ??Ï≤òÎ¶¨ ??Ï¶âÏãú Î≥µÍ?
            try:
    ...
    def _sync_flag_leds(self) -> None:
        """?ÑÏû¨ Z/N/V Í∞íÏùÑ ÏßÄ?ïÎêú ??LED??Î∞òÏòÅ"""
    ...
    def run_led(self) -> None:
        """LED Ïª®Ìä∏Î°??åÎ†à?∏Ïóê ?òÌï¥ Í≤åÏù¥?ÖÎêò???§Ìñâ Î£®ÌîÑ.
        grave/esc/tab/caps/left_shift ??Ïª¨Îü¨Î•?Ï£ºÍ∏∞?ÅÏúºÎ°??êÎèÖ?òÏó¨
        RUN/PAUSE/HALT/R_SOFT/R_HARD/STEP/.. µø¿€¿ª LED ªˆ¿∏∑Œ ∆Æ∏Æ∞≈«’¥œ¥Ÿ.
        """
    ...
    def _soft_reset_visuals(self) -> None:
        """?ÑÎ°úÍ∑∏Îû®?Ä ?†Ï???Ï±?PC/IR/Flags Î∞??úÏãúÎ•?Ï¥àÍ∏∞??"""
        self.pc.reset()
        self.halted = False
        self.ir.clear()
        self.flags["Z"] = 0
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if not cont:
                break
        # ?§Ìñâ Ï¢ÖÎ£å Ï¶âÏãú ?®Í≥Ñ ?úÏãú ?åÎì±(WRITEBACK ?¨Ìï®)
        try:
            clear_stages()
        except Exception:
            pass
    ...
    def _write_u8_to_group(self, grp: str, u8: int):
        ...
        u8 &= 0xFF
        width = len(labels)
        # labels[-1]??LSBÍ∞Ä ?òÎèÑÎ°???ù∏?±Ïã±
        for i in range(width):  # i=0..7 -> ÎπÑÌä∏ i
            bit = (u8 >> i) & 1
            lab = labels[width - 1 - i]
            self.mem.set(lab, bit)
    ...
    def _read_u8_from_group(self, grp: str) -> int:
        ...
        width = len(labels)
        val = 0
        # labels[-1]??LSB ????ù∏?±Ïã±?ºÎ°ú ?ΩÏñ¥??iÎ≤àÏß∏ ÎπÑÌä∏Î°?
    ...
    def _exec_one(self, op: str, args: tuple[Any, ...]) -> Dict[str, int]:
        ...
        if op == "SHL":
            ...
            self.flags["C"] = 1 if (_to_u8(a) & 0x80) else 0
            # SHL overflow: ÏµúÏÉÅ??ÎπÑÌä∏ Î≥Ä???¨Î?Î°??êÎã®
            ov = 1 if (_sign_bit(a) != _sign_bit(v)) else 0
            self.mem.set(dst_name, v)
            self.flags["V"] = ov
            _set_zn_from_val(self.flags, v)
        ...
        if op == "SHR":
            ...
            # Carry out from LSB
            self.flags["C"] = 1 if (_to_u8(a) & 0x01) else 0
            self.flags["V"] = 0 # ASR?Ä ?§Î≤Ñ?åÎ°ú???ÜÏùå
            _set_zn_from_val(self.flags, v)
    ...
    def _on_decode(self, decoded: Tuple[str, tuple[Any, ...]]) -> None:
        op, args = decoded
        # Stage: DECODE (ÎπÑÎèôÍ∏??úÏãú)
        try:
            post_stage("DECODE")
        except Exception:
            pass
        self._println(f"[DECODE] {op} {args}")
        # IR??FETCH ?®Í≥Ñ?êÏÑú Í≥†ÏàòÏ§Ä ?ºÏù∏ Í∏∞Ï??ºÎ°ú ?¥Î? ?úÏãú??
    def _on_execute(self, desc: str) -> None:
        # Stage: EXECUTE (ÎπÑÎèôÍ∏??úÏãú)
        try:
            post_stage("EXECUTE")
        except Exception:
            pass
    ...
    def _on_writeback(self, changes: Dict[str, int]) -> None:
        if changes:
            ...
            # Stage: WRITEBACK (ÎπÑÎèôÍ∏??úÏãú: Î≥ÄÍ≤??ÜÏñ¥???ôÏùº)
            try:
                post_stage("WRITEBACK")
            except Exception:
                pass
